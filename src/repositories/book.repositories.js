...
db.run(`CREATE TABLE IF NOT EXISTS books(
    ...
    title TEXT NOT NULL,
    euthor TEXT NOT NULL,
    userId INTEGER,
    FOREING KEY (userId) REFERENCES user(id)
)`);

function creatBookRepository(newBook, userId){
    return new Promise((resolve, reject) =>{
        const { title, author } = newBook;
        db.run( 
            `INSERT INTO book (title, author, userId) VALUES (?, ?, ?)`,
            [title, author, userId],
            function (err) {
                if (err) {
                    reject(err);
    ...
    return new Promise((resolve, reject) => {
       db.run(`DELETE FROM books WHERE id = ?`, [bookId], function (err) {
          if (err) {
           ...
           resolve({mesage: "Book deleted sucessfully", bookId});
          }
        });
    });
};
